...
q_fun <- function(bb, df){
  ...
  #bb_vector = c(b0 + b1*xx)
  #q <- -sum(yy*rr*log(1 + exp(-b0-b1*xx)) + (1-rr)*log(1+exp(b0+b1*xx)) + sigmoid(bb_minus, xx)*log(1 + exp(-b0-b1*xx)) + sigmoid(bb_plus, xx)*log(1+exp(b0+b1*xx)))
  q <- sum((rr*yy*(b0+b1*xx)) - log(1 + exp(b0+b1*xx))  )#+ (1 - rr)*(b0 + b1*xx)*sigmoid(bb, xx))
  return(q)
}
